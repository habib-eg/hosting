...
namespace Habib\Hosting\Host\DirectAdmin;

use App\Tools\IdnaConvertor;
use App\Tools\HTTPSocket;
use Habib\Hosting\Base\HostingInterface;
use Illuminate\Http\Response;

class DirectAdmin implements HostingInterface{
...
//            return HostingAPI::ANSWER_OK;
        ...
        elseif(isset($result['details'])) {
            if($result['details'] == 'Package not found'){
                throw  new \Exception('ANSWER PLAN NOT EXIST',404);
            } elseif($result['details'] == 'Invalid Email Address'){
                throw  new \Exception('ANSWER USER EMAIL NOT VALID',422);
            } elseif ($result['details'] == 'That username already exists on the system'){
                throw  new \Exception('ANSWER USER ALR ADY EXIST',400);
            } elseif ($result['details'] == 'Invalid Password'){
                throw  new \Exception('ANSWER USER PASSWORD NOT VALID',422);
            }
...
//            return HostingAPI::ANSWER_OK;
    ...
    public function userExist($user){
        //http://www.directadmin.com/api.html#showallusers GET/POST
